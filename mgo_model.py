...
class MgoModel:
    ...
    def map_with_pv(self, latitude, longitude):
    ...
    def run_model(self, minimum_area_m2, demand_multiplier, price_pv_multiplier, price_wire, price_conn, price_maintenance, years, max_tot_length):
        ...
        village_map = folium.Map([y_mean, x_mean], zoom_start=15, control_scale=True)
        icon = folium.Icon(icon='bolt', color='green', prefix='fa')
        folium.Marker([self.gen_lat, self.gen_long], icon=icon, popup='PV plant location').add_to(village_map)
        
        folium.LayerControl(collapsed=False, position='topleft').add_to(village_map)

        colormap = linear.BuGn.scale(0, 300)
        colormap.caption = 'Household annual demand (USD)'
        colormap.add_to(village_map)

        colormap2 = linear.OrRd.scale(0, 5000)
        ...
        if len(network_wgs84.index) > 0 and len(buildings_wgs84.index) > 0:
            ...
            styles = []
            for index, row in buildings_wgs84.iterrows():
                if row['tot_dist'] > 4000:
                    fill_color = '#b30000'
                elif row['tot_dist'] > 2000:
                    fill_color = '#e34a33'
                elif row['tot_dist'] > 1000:
                    fill_color = '#fc8d59'
                elif row['tot_dist'] > 500:
                    fill_color = '#fdcc8a'
                else:
                    fill_color = '#fef0d9'
                styles.append({'fillColor': fill_color, 'weight': 1, 'color': 'black', 'fillOpacity': 1})
        ...
        file_tag = '{}_{}_{}_{}_{}_{}_{}_{}'.format(self.village, minimum_area_m2, demand_multiplier, price_pv_multiplier, price_wire, price_conn, price_maintenance, years)
