...
def test_clusters():
	...
	assert all(clusters.columns == ['geometry', 'area_m2'])

	clusters = clustering.cluster_pops(clusters, RASTER_FILE)
	assert all(clusters.columns == ['geometry', 'area_m2', 'pop_sum'])

	clusters = clustering.cluster_grid_distance(clusters, GRID_FILE, clipped[0].shape, affine)
	assert all(clusters.columns == ['geometry', 'area_m2', 'pop_sum', 'grid_dist'])
