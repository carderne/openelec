...
def priority(
    ...
    if pop_range:
        clusters.loc[
            ~clusters["pop"].between(pop_range[0], pop_range[1]), "consider"
        ] = 0
    if grid_range:
        clusters.loc[
            ~clusters["grid"].between(grid_range[0], grid_range[1]), "consider"
        ] = 0
    if ntl_range:
        clusters.loc[
            ~clusters["ntl"].between(ntl_range[0], ntl_range[1]), "consider"
        ] = 0
    if gdp_range:
        clusters.loc[
            ~clusters["gdp"].between(gdp_range[0], gdp_range[1]), "consider"
        ] = 0
    if travel_range:
        clusters.loc[
            ~clusters["travel"].between(travel_range[0], travel_range[1]), "consider"
        ] = 0
